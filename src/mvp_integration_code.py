...
"""
INSTRUCTIONS TO INTEGRATE INTO master_luciq_api.py:

1. Add the above code to the top of master_luciq_api.py (after existing imports)

2. Add these endpoints to the FastAPI app:
...
# MVP Revenue-Generating Endpoints
...
async def mvp_pain_point_detection(request: PainPointAnalysisRequest, auth: Dict = Depends(get_mvp_api_key_auth)):
    ...
    try:
        ...
        await mvp_api_key_service.track_mvp_usage(
            auth["api_key_hash"], "/api/mvp/pain-point-detection", response_time, 200
        )
        ...
        response_time = int((datetime.now() - start_time).total_seconds() * 1000)
        await mvp_api_key_service.track_mvp_usage(
            auth["api_key_hash"], "/api/mvp/pain-point-detection", response_time, 500
        )
        raise HTTPException(status_code=500, detail=str(e))
...
async def mvp_market_validation(request: MarketValidationRequest, auth: Dict = Depends(get_mvp_api_key_auth)):
    ...
    try:
        ...
        response_time = int((datetime.now() - start_time).total_seconds() * 1000)
        await mvp_api_key_service.track_mvp_usage(
            auth["api_key_hash"], "/api/mvp/market-validation", response_time, 200
        )
        ...
        response_time = int((datetime.now() - start_time).total_seconds() * 1000)
        await mvp_api_key_service.track_mvp_usage(
            auth["api_key_hash"], "/api/mvp/market-validation", response_time, 500
        )
        raise HTTPException(status_code=500, detail=str(e))
...
async def mvp_competitive_analysis(request: SolutionGapAnalysisRequest, auth: Dict = Depends(get_mvp_api_key_auth)):
    ...
    try:
        ...
        response_time = int((datetime.now() - start_time).total_seconds() * 1000)
        await mvp_api_key_service.track_mvp_usage(
            auth["api_key_hash"], "/api/mvp/competitive-analysis", response_time, 200
        )
        ...
        response_time = int((datetime.now() - start_time).total_seconds() * 1000)
        await mvp_api_key_service.track_mvp_usage(
            auth["api_key_hash"], "/api/mvp/competitive-analysis", response_time, 500
        )
        raise HTTPException(status_code=500, detail=str(e))
...
async def mvp_business_signals(hours_back: int = 24, auth: Dict = Depends(get_mvp_api_key_auth)):
    ...
    try:
        ...
        response_time = int((datetime.now() - start_time).total_seconds() * 1000)
        await mvp_api_key_service.track_mvp_usage(
            auth["api_key_hash"], "/api/mvp/business-signals", response_time, 200
        )
        ...
        response_time = int((datetime.now() - start_time).total_seconds() * 1000)
        await mvp_api_key_service.track_mvp_usage(
            auth["api_key_hash"], "/api/mvp/business-signals", response_time, 500
        )
        raise HTTPException(status_code=500, detail=str(e))
...
REVENUE POTENTIAL:
- Starter: $49/month × 20 customers = $980 MRR
- Professional: $149/month × 50 customers = $7,450 MRR  
- Business: $299/month × 30 customers = $8,970 MRR
- Enterprise: $999/month × 10 customers = $9,990 MRR

TOTAL POTENTIAL: $27,390 MRR ($328K ARR) with just 110 customers

NEXT STEPS:
1. Add this code to master_luciq_api.py
2. Test the MVP endpoints
